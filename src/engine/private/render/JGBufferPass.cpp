...
void JGBufferPass::Execute(const JRenderPassContext& context, const JFrameDesc& frameDesc)
{
	...
	for (uint32 drawItemIndex : frameDesc.opaqueDrawItemIndices)
	{
		...
		if (resources.mesh == nullptr || resources.transform == nullptr)
		{
		...
		// material 셰이더로 표면을 기술하고, 준비되지 않은 material은 pass 기본 셰이더로 폴백한다.
		const bool hasMaterialPipeline = resources.material != nullptr
			&& resources.material->shader != nullptr
			&& resources.material->pipeline != nullptr;
		Render::JShader* shader = hasMaterialPipeline ? resources.material->shader : _shader;
		Render::JPipeline* pipeline = hasMaterialPipeline ? resources.material->pipeline : _pipeline;

		Render::JGraphicResource graphicResource(shader);
		...
		context.commandQueue->SetPipeline(pipeline);
