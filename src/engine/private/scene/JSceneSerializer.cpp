...
namespace
{
	...
	JSceneEntityData loadEntity(const json& value)
	{
		...
		if (value.contains("light") && value.at("light").is_object())
		{
			...
			data.light.type = light.value("type", "point") == "directional"
				? JSceneLightType::Directional
				: JSceneLightType::Point;
	...
	json saveEntity(const JSceneEntityData& data)
	{
		...
		if (data.hasLight)
		{
			value["light"] =
			{
				...
				{ "type", data.light.type == JSceneLightType::Directional ? "directional" : "point" },
