...
uint32 JHashFunction::CRCTablesSB8[8][256] =
{
	{
...
uint32 JHashFunction::MemCrc32(const void* data, uint32 length, uint32 crc/*=0 */)
{
	// Based on the Slicing-by-8 implementation found here:
	// http://slicing-by-8.sourceforge.net/

	crc = ~crc;
	const uint8* __restrict binData = (uint8*)data;

	// First we need to align to 32-bits
	int32 initBytes = static_cast<int32>(Align(binData, 4) - binData);

	if (static_cast<int64>(length) > initBytes)
	{
		length -= initBytes;

		for (; initBytes; --initBytes)
		{
			crc = (crc >> 8) ^ CRCTablesSB8[0][(crc & 0xFF) ^ *binData++];
		}

		auto data4 = (const uint32*)binData;
